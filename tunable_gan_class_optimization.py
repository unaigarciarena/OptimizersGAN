...
def next_random_batch(num, data, i):
    """
...
class GAN:
    ...
    def training_definition(self, seed=None, x=0, y=0, z=0, w=0):
        # =============================== TRAINING ====================================
        if seed is not None:
        ...
        #self.g_solver = tf.train.AdadeltaOptimizer(learning_rate=self.descriptor.l_rate).minimize(
        #    self.G_loss, var_list=[self.Gen_network.List_weights, self.Gen_network.List_bias])

        #self.d_solver = tf.train.AdadeltaOptimizer(learning_rate=self.descriptor.l_rate).minimize(
        #    self.D_loss, var_list=[self.Disc_network.List_weights, self.Disc_network.List_bias])

        #self.g_solver = tf.train.AdagradOptimizer(learning_rate=self.descriptor.l_rate).minimize(
        #    self.G_loss, var_list=[self.Gen_network.List_weights, self.Gen_network.List_bias])

        #self.d_solver = tf.train.AdagradOptimizer(learning_rate=self.descriptor.l_rate).minimize(
        #    self.D_loss, var_list=[self.Disc_network.List_weights, self.Disc_network.List_bias])

        #self.g_solver = tf.train.RMSPropOptimizer(learning_rate=self.descriptor.l_rate).minimize(
        #    self.G_loss, var_list=[self.Gen_network.List_weights, self.Gen_network.List_bias])

        self.d_solver = tf.train.RMSPropOptimizer(learning_rate=self.descriptor.l_rate).minimize(
            self.D_loss, var_list=[self.Disc_network.List_weights, self.Disc_network.List_bias])

        #self.g_solver = tf.train.MomentumOptimizer(learning_rate=self.descriptor.l_rate, momentum=0.9).minimize(
        #    self.G_loss, var_list=[self.Gen_network.List_weights, self.Gen_network.List_bias])

        #self.d_solver = tf.train.MomentumOptimizer(learning_rate=self.descriptor.l_rate, momentum=0.9).minimize(
        #    self.D_loss, var_list=[self.Disc_network.List_weights, self.Disc_network.List_bias])

        #self.g_solver = tf.train.MomentumOptimizer(learning_rate=self.descriptor.l_rate, momentum=0.9, use_nesterov=True).minimize(
        #    self.G_loss, var_list=[self.Gen_network.List_weights, self.Gen_network.List_bias])

        #self.d_solver = tf.train.MomentumOptimizer(learning_rate=self.descriptor.l_rate, momentum=0.9, use_nesterov=True).minimize(
        #    self.D_loss, var_list=[self.Disc_network.List_weights, self.Disc_network.List_bias])


        self.g_solver = tf.train.FtrlOptimizer(learning_rate=self.descriptor.l_rate, learning_rate_power=-x, initial_accumulator_value=y, l1_regularization_strength=z, l2_regularization_strength=w).minimize(
            self.G_loss, var_list=[self.Gen_network.List_weights, self.Gen_network.List_bias])

        #self.d_solver = tf.train.FtrlOptimizer(learning_rate=self.descriptor.l_rate).minimize(
        #    self.D_loss, var_list=[self.Disc_network.List_weights, self.Disc_network.List_bias])
    ...
    def running(self, batch_type, data, mb_size, number_iterations, nsamples, print_cycle):
        ...
        for it in range(number_iterations):
            ...
            _, d_loss_curr = self.sess.run([self.d_solver, self.D_loss], feed_dict={self.X: x_mb, self.Z: z_mb})
            _, g_loss_curr = self.sess.run([self.g_solver, self.G_loss], feed_dict={self.Z: z_mb})
        ...
        samples = self.sess.run(self.g_sample, feed_dict={self.Z: self.sample_z(nsamples, self.descriptor.z_dim)})
